...
impl Index {
    ...
    fn index_locate(&mut self, hash: &Hash) -> Option<QueueEntry> {
        ...
        let result_opt = self.select1(&format!("SELECT id, height, payload, blob_ref FROM \
                                                hash_index WHERE hash=x'{}'",
                                               hash.bytes.to_hex()));
    ...
    fn insert_completed_in_order(&mut self) {
        let mut insert_stm = self.dbh
                                 .prepare("INSERT INTO hash_index (id, hash, height, payload, \
                                           blob_ref) VALUES (?, ?, ?, ?, ?)",
                                          &None)
                                 .unwrap();
    ...
    fn set_tag(&mut self, id_opt: Option<i64>, tag: tags::Tag) {
        ...
        self.exec_or_die(&format!("UPDATE hash_index SET tag={:?} {}", tag as i64, condition)[..]);
    }
    ...
    fn list_ids_by_tag(&mut self, tag: i64) -> Vec<i64> {
        ...
        let mut cursor =
            self.prepare_or_die(&format!("SELECT id FROM hash_index WHERE tag={:?} ORDER BY \
                                          height DESC",
                                         tag)[..]);
    ...
    fn read_gc_data(&mut self, hash_id: i64, family_id: i64) -> GcData {
        match self.select1(&format!("SELECT gc_int, gc_vec FROM gc_metadata
            ...
            Some(row) => {
                GcData {
                    num: row.get_i64(0),
                    bytes: row.get_blob(1).unwrap_or(&[]).to_vec(),
                }
            }
        }
    }
    ...
    fn update_family_gc_data(&mut self,
        ...
        {
            let mut select_stm = self.prepare_or_die("SELECT hash_id FROM gc_metadata WHERE \
                                                      family_id=?");
    ...
    fn delete_gc_data(&mut self, hash_id: i64, family_id: i64) {
        self.exec_or_die(&format!("DELETE FROM gc_metadata WHERE hash_id={:?} AND \
                                   family_id={:?}",
                                  hash_id,
                                  family_id)[..]);
    }
    ...
    fn delete(&mut self, id: i64) {
        let mut cursor = self.prepare_or_die(&format!("DELETE FROM hash_index WHERE id={:?} \
                                                       LIMIT 1",
                                                      id));
        assert_eq!(cursor.step(), SQLITE_DONE);
        let mut cursor2 = self.prepare_or_die(&format!("DELETE FROM gc_metadata WHERE \
                                                        hash_id={:?}",
                                                       id));
